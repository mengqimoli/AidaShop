...
function orderDomain(url, ordername, ordertype) {
	alert(url);
	alert(ordername);
	alert(ordertype);
	$("#domainForm").attr("action",url+".action?orderName="+orderName+"&orderType"+orderType);//修改地址
	$("#domainForm").submit();
	$("#domainForm").attr("action",url+".action");//查询地址
}
// 高级修改
